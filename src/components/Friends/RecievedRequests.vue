    ...
    >
      <md-list class="md-double-line" v-if="list.length>0">
        <md-subheader>
          ...
          <h3 class="md-accent-font">{{$t("main.recieved_friend_requests")}}</h3>
        </md-subheader>
        <md-list-item v-for="(item,index) in list" :key="index">
          ...
          <md-button @click="rejectRequest(index)" class="md-icon-button md-list-action">
            <md-icon>cancel</md-icon>
          </md-button>
          <md-button @click="acceptRequest(index)" class="md-icon-button md-list-action">
      ...
      <md-empty-state
        ...
        md-rounded
        md-icon="group"
        :md-label="$t('main.no_requests')"
        :md-description="$t('main.no_requests_desc')"
      ></md-empty-state>
      <md-snackbar
        md-position="center"
        :md-duration="4000"
...
export default {
  ...
  methods: {
    getRequests() {
      this.$Progress.start();
      this.$http({
        url: APIURL + "get-recieved-request"
      })
        .then(response => {
    ...
    rejectRequest(index){
      ...
      this.$http({
        url: APIURL + "reject-request",
        method:"POST",
        data:{
          sender_id:this.list[index].id
        },
        headers: JSONHeader
      }).then(response => {
        console.log(response.data.data);
