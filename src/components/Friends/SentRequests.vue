    ...
    >
      <md-list class="md-double-line" v-if="list.length>0">
        <md-subheader>
          ...
          <h3 class="md-accent-font">{{$t("main.sent_friend_requests")}}</h3>
        </md-subheader>
        <md-list-item v-for="(item,index) in list" :key="index">
          ...
          <md-button @click="removeRequest(index)" class="md-icon-button md-list-action">
...
export default {
  ...
  methods: {
    ...
    removeRequest(index) {
      ...
      this.$http({
        url: APIURL + "cancell-request",
        method: "POST",
        data: {
          recipient_id: this.list[index].id
        },
        headers: JSONHeader
      }).then(response => {
        console.log(response);
